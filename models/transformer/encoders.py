...
class MultiLevelEncoder(nn.Module):
    ...
    def forward(self, input, attention_weights=None):
        ...
        attention_mask = (torch.sum(input, -1) == self.padding_idx).unsqueeze(1).unsqueeze(1) # (b_s, 1, 1, seq_len) #mask padding tokens

        outs = []
        out = input
        for l in self.layers:
            out = l(out, out, out, attention_mask, attention_weights) #query, keys, values
